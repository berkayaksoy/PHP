...
class DynamoDB
{
	...
	{
		return new DynamoDBReader($id, $this->client, $this->s3Client, [
			"TableName" => $this->config['leosdk']['resources']['LeoStream'],
	...
	{
		$leoEventTable = $this->config['leosdk']['resources']['LeoEvent'];
		$leoCronTable = $this->config['leosdk']['resources']['LeoCron'];
		...
		if (!empty($result['UnprocessedKeys'])) {
			...
			if (empty($responses) || empty($responses[$leoEventTable])) {
				return null;
			} else {
				$leoEvent = $this->marshaler->unmarshalItem($responses[$leoEventTable][0]);
			}

			//Check if the cron bot doesn't exist yet
			if (empty($responses[$leoCronTable])) {
				...
				$result = $this->client->putItem([
					"TableName" => $leoCronTable,
					"Item" => $this->marshaler->marshalItem($leoCron)
				]);
				$compareStart = null;
			} else {
				$leoCron = $this->marshaler->unmarshalItem($responses[$leoCronTable][0]);
...
class DynamoDBReader extends EventIterator
{
	...
	public function __construct($id, $client, $s3Client, $params, $range, $opts)
	{
		...
		$this->events = new DynamoDBIterator($client, $s3Client, $params, $range['start'], $opts);
	}
	...
	public function checkpoint($check = null)
	{
		...
		$result = $this->client->updateItem([
			'TableName' => $this->config['leosdk']['resources']['LeoCron'],
			"Key" => [
				"id" => ['S' => $this->id]
			],
...
class DynamoDBIterator implements \Iterator
{
	private $client;
	private $s3Client;
